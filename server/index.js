...
app.get('/api/dashboard', basicAuth, (req, res) => {
  try {
    ...
    const plants = db.prepare(
      'SELECT * FROM plants WHERE projectId = ?'
    ).all(projectId);

    const compSnapshot = db.prepare(
      'SELECT * FROM compliance_snapshot WHERE projectId = ?'
    ).all(projectId);
    ...
    const msgAchievement = db.prepare(`
      SELECT plant, annualMSG, ytdActual
      FROM volume_ld
      WHERE projectId = ?
    `).all(projectId);

    const revProjection = db.prepare(`
      SELECT *
